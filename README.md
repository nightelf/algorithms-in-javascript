...
| [Math](./math) | [`findMissingSequentialNumber`](./math/missing-sequential-number.ts) — the absent value in a run of consecutive integers | O(n) time, O(1) space |
| [Math](./math) | [`isValidSequentialList`](./math/missing-sequential-number.ts) — whether a list satisfies that run's contract | O(n) time, O(n) space |
