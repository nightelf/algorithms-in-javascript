 ...
 *
 * @param {string} enclosingPairs alternating opener/closer characters
 * @returns {{ open: Map<string, string>, close: Map<string, string> }}
 * @throws {Error} if the string is empty, odd-length, or repeats a character
 */
const buildLookups = (enclosingPairs) => {
 ...
 * @param {string} [enclosingPairs] alternating opener/closer characters
 * @returns {boolean}
 * @throws {Error} if `enclosingPairs` is empty, odd-length, or repeats a character
 */
export const hasValidEnclosingPairs = (testString, enclosingPairs = DEFAULT_ENCLOSING_PAIRS) => {
  const { open, close } =
